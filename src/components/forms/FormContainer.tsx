...
const FormContainer = ({
  ...
  const { touched, handleBlur, errors, handleChange, handleSubmit, values } =
    useFormik({
      initialValues,
  ...
  useEffect(() => {
    ...
    const noErrors = isLogin
      ? !errors.email && !errors.password
      : !errors.email && !errors.password && !errors.username;
    setDisabled(!(allFieldsFilled && noErrors));
  }, [values, errors]);

  const renderUnderlinedText = (label: string) => (
    <span className="text-text-dark underline">{label}</span>
  );

  const renderFormHeader = () => {
    return (
      <div className="flex flex-col justify-center lg:items-center">
        <h1 className="text-default-text-color text-2xl md:text-3xl font-medium ">
          {title}
        </h1>
        <p className="text-default-text-color font-albert">
          {subtitle}{" "}
          <span
            className="underline text-text-dark hover:text-blue-700 cursor-pointer"
    ...
    return (
      <div className="flex w-full max-w-[340px] p-2 pl-0 mt-7">
        <p className="text-field-label md:text-base text-sm">
          ...
          {renderUnderlinedText("Terms of use")} and{" "}
          {renderUnderlinedText("Privacy Policy")}
        </p>
      </div>
    );
  };
      ...
      <div className="flex flex-col gap-8 w-full">
        <div className="flex lg:items-center lg:justify-center">
          <img
            src="/svg/logo.svg"
            alt=""
            className="lg:w-[109px] lg:h-9 w-[89px] h-7 md:h-8 md:w-[102px]"
          />
        </div>
        <form onSubmit={handleSubmit} className="flex flex-col">
          ...
          {renderFormFields()}
          {error.length > 0 && (
            <p className="text-red-500 mt-3 text-">{error}</p>
          )}
          {renderTermsField()}
          <Button
            type="submit"
          ...
          <Button
            ...
            disabled
            className="bg-white text-black border border-[#bbb]"
