...
const PersonalForm = ({ uid, email }: PersonalForm) => {
  ...
  const { handleBlur, handleChange, handleSubmit, values, errors, touched ,setFieldValue} = useFormik({
    initialValues: { firstName: "", lastName: "", email, phone: "",imageUrl:"" },
    onSubmit: async (values) => {
      const response = await fetch(`${process.env.NEXT_PUBLIC_URL}/api/users/update-user/${uid}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ firstName: values.firstName, lastName: values.lastName, phone: values.phone,imageUrl:values.imageUrl }),
      });
    ...
    return (
      <div className="flex flex-col gap-9 w-full">
        <div className="flex items-center gap-12 w-full justify-center">
          <TextField handleBlur={handleBlur} handleChange={handleChange} id="firstName" label="First Name" isPassword={false} login={false} name="firstName" touched={touched.firstName ?? false} type="text" error={errors.firstName} value={values.firstName} />
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-11 w-full items-start">
      ...
      {renderFormFields()}
      <Button disabled={false} label="Create an account" type="submit" className="bg-black text-white" />
    </form>
  );
};
